...
**`viewers/`** — multi-neuron grid CLIs, thin wrappers over `explore.grid`: `psth.py`, `raster_plot.py`, `autocorrelogram.py`, `browser.py`, and a family of "firing rate vs per-trial regressor" plots that share one core (`explore._draw_fr_vs_regressor`): `firing_rate_vs_perc_p.py` (x = behavioural rolling P(reward)), `firing_rate_vs_hgf_p.py` (x = HGF latent belief p̂), `firing_rate_vs_delta1.py` (x = HGF prediction error δ₁ = outcome − p̂, gamble trials only), and `firing_rate_vs_rw_pe.py` (x = RW+stickiness prediction error δ = outcome − Q, gamble trials only — the best-fitting choice model by BIC, see `results/hgf/model_comparison.csv`). The HGF viewers read `results/hgf/trajectory_<session>.csv` (run `python -m analysis.hgf.run`); the RW viewer reads `results/rw/trajectory_<session>.csv` (run `python -m analysis.rw_trajectories`). All accept `by_condition=True` (CLI `--by-condition`, `--pooled` for the δ viewers) to colour points by outcome (G+R/G+N/S+R) and fit one regression per condition.
